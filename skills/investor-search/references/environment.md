...
**All writes go through `scripts/store.py`** (`SKILL.md`, *ChatGPT and Codex*); this file describes what
...
**Step 2 is how a user who cannot open the path still gets the file.** In ChatGPT that is a
download offered in the reply; in a Codex cloud task it is the task's result; in Codex on the
user's machine the path is all there is (`SKILL.md`, *ChatGPT and Codex*). Check which one this
...
**Writing silently goes somewhere else.** In ChatGPT and Codex cloud this is a session whose files are not kept after it ends, or a local Codex and a cloud task with different working directories: the write succeeds and the next run looks somewhere else. The symptom is
...
does not (`SKILL.md`, *Before Job 0* §6). The fix the user can make is on the ChatGPT and Codex side: a folder
that persists — the user's own directory in Codex on their machine — for each user or team.

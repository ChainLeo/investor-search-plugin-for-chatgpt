...
`investor-search/lists-local.md` (*ChatGPT and Codex* below), and
...
## ChatGPT and Codex — how this skill runs here
...
```
Codex on the user's         the cwd is the user's own folder and it persists. Use it, and
machine (CLI, IDE)          say in the first reply which absolute path you are writing to.
a Codex cloud task          the container is thrown away when the task ends. Files survive
                            only if they are part of the task's result. Say so in the first
                            reply; the Excel file is the memory for the next run.
ChatGPT, or any session     files are not kept between conversations. Say so in the first
where files do not last     reply, in one sentence, before searching: the Excel file is the
                            ...
                            Report PARTIAL, never a finished market (§7 tier B).
...
```
ChatGPT             run deliver with --out set to a folder this session can offer for
                    download, and give the user that file as a download in the reply.
                    ONE file, <market>-investors.xlsx. Never attach the CSVs: they are
                    the memory, not the deliverable. Hand it over WITHOUT asking.
Codex cloud task    make the file part of the task's result and name it.
Codex on the        state the absolute path.
user's machine
...
let the shell place them in the working directory (*ChatGPT and Codex* above); where that folder persists, the files are still there on a later run. In a shared workspace the same layout sits one level down, under
...
Ask only what §5 and §6 require, together, in one question where possible.
...
2  deliver the file itself when the user cannot open      a chat channel;
   that path — a download in the reply (*ChatGPT and Codex*)  automatically, never ask first;
...
[ ] the .xlsx reached the user: a download (ChatGPT), the task result (Codex cloud), absolute path (Codex CLI),

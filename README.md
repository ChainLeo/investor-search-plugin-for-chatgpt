...
Add **Investor Search** from the Plugins directory in ChatGPT or Codex, or drop the
`investor-search` folder into your Codex skills directory. Do not edit the
...
your own working directory when Codex runs on your machine) — and looks there first:
...
**One thing to know about where the files go.** With Codex on your machine they land in your
own working directory and stay there. In ChatGPT and in Codex cloud tasks they are **not kept
after the session** — the skill says so before it starts, and the Excel file becomes the memory:
upload it next time and the run continues from it.
